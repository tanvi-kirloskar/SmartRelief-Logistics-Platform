...
import { MapPin, Users, Building2, Heart, AlertTriangle, Download } from 'lucide-react'
...
export function IndiaMap() {
  ...
  const FILTER_BTNS = [
    { key: 'rescue', label: 'Rescue Teams', color: '#22c55e' },
    { key: 'hospital', label: 'Hospitals', color: '#3b82f6' },
    { key: 'ngo', label: 'NGOs', color: '#f59e0b' },
    { key: 'incident', label: 'Active Incidents', color: '#ef4444' },
  ]
    ...
    <div className="rounded-xl overflow-hidden" style={{ background: '#081221', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold tracking-widest text-white/40 uppercase">India Operations Map</span>
        </div>
        <div className="flex items-center gap-2">
          {FILTER_BTNS.map(fb => (
            <button
              key={fb.key}
              onClick={() => toggleMapFilter(fb.key as any)}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-medium transition-all"
              style={{
                background: mapFilters[fb.key as keyof typeof mapFilters] ? `${fb.color}15` : 'transparent',
                border: `1px solid ${mapFilters[fb.key as keyof typeof mapFilters] ? fb.color : 'rgba(255,255,255,0.1)'}`,
                color: mapFilters[fb.key as keyof typeof mapFilters] ? fb.color : 'rgba(255,255,255,0.4)',
              }}
            >
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: fb.color }} />
              {fb.label}
            </button>
          ))}
          <button className="w-7 h-7 rounded flex items-center justify-center text-white/30 hover:text-white/60"
      ...
      <div className="flex" style={{ height: '460px' }}>
        {/* Map SVG */}
        <div className="flex-1 relative" style={{ height: '100%', background: 'linear-gradient(135deg, #020817 0%, #061020 100%)' }}
          onClick={() => setSelected(null)}>
          <svg
            viewBox="100 0 750 800"
...
>
            {/* Grid lines */}
            {[...Array(8)].map((_, i) => (
              <line key={`v${i}`} x1={100 + i * 110} y1={0} x2={100 + i * 110} y2={800}
                stroke="rgba(34,197,94,0.04)" strokeWidth="1" strokeDasharray="4,8" />
            ))}
            {[...Array(9)].map((_, i) => (
              <line key={`h${i}`} x1={100} y1={i * 100} x2={850} y2={i * 100}
                stroke="rgba(34,197,94,0.04)" strokeWidth="1" strokeDasharray="4,8" />
            ))}

            {/* India outline */}
            <path d={INDIA_PATH} fill="rgba(34,197,94,0.04)" stroke="#22c55e" strokeWidth="1.5" strokeOpacity="0.3" />

            {/* City labels */}
            {[
              { name: 'Delhi', lat: 28.6, lng: 77.2 },
              { name: 'Mumbai', lat: 19.0, lng: 72.8 },
              { name: 'Kolkata', lat: 22.5, lng: 88.4 },
              { name: 'Chennai', lat: 13.1, lng: 80.3 },
              { name: 'Nagpur', lat: 21.1, lng: 79.1 },
              { name: 'Patna', lat: 25.6, lng: 85.1 },
            ].map(city => {
              const pos = latLngToSVG(city.lat, city.lng)
              return (
                <text key={city.name} x={pos.x + 14} y={pos.y + 4}
            ...
            {/* Markers */}
            {filtered.map(m => (
              <MarkerDot key={m.id} marker={m} onClick={handleMarkerClick} />
            ))}
            ...
            {selected && (() => {
              ...
              const flipX = px > 650
              const flipY = py > 550
              const ox = flipX ? -200 : 15
              const oy = flipY ? -130 : 15
              return (
                <foreignObject x={px + ox} y={py + oy} width="190" height="120">
                  <div style={{
                    background: '#081221',
                    ...
                    <div style={{ fontWeight: 700, marginBottom: 4, color: MARKER_COLORS[selected.type] }}>
                      {selected.title}
                    </div>
                    <div style={{ color: 'rgba(255,255,255,0.5)', marginBottom: 2 }}>
                      Type: <span style={{ color: 'rgba(255,255,255,0.8)' }}>{selected.type}</span>
                    </div>
                    {selected.severity && (
                      <div style={{ color: 'rgba(255,255,255,0.5)', marginBottom: 2 }}>
                        Severity: <span style={{ color: '#ef4444' }}>{selected.severity}</span>
                      </div>
                    )}
                    {selected.peopleImpacted && (
                      <div style={{ color: 'rgba(255,255,255,0.5)', marginBottom: 2 }}>
                        Impacted: <span style={{ color: 'rgba(255,255,255,0.8)' }}>{selected.peopleImpacted.toLocaleString()}</span>
                      </div>
                    )}
                    {selected.status && (
                      <div style={{ color: 'rgba(255,255,255,0.5)' }}>
                        Status: <span style={{ color: '#22c55e' }}>{selected.status}</span>
                      </div>
                    )}
                  </div>
                </foreignObject>
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-1">
            <button className="w-7 h-7 rounded flex items-center justify-center text-white/60 hover:text-white transition-colors text-lg font-mono"
              style={{ background: '#081221', border: '1px solid rgba(255,255,255,0.1)' }}>+</button>
            <button className="w-7 h-7 rounded flex items-center justify-center text-white/60 hover:text-white transition-colors text-lg font-mono"
              style={{ background: '#081221', border: '1px solid rgba(255,255,255,0.1)' }}>−</button>
          </div>
        </div>
        ...
        {/* Right panel */}
        <div className="w-52 flex-shrink-0 p-4 flex flex-col gap-4" style={{ borderLeft: '1px solid rgba(255,255,255,0.06)' }}>
          <div>
            <span className="text-[10px] font-semibold tracking-widest text-white/30 uppercase">Active Assets</span>
            <div className="flex flex-col gap-2 mt-3">
              {ACTIVE_ASSETS.map(a => {
                const Icon = a.icon
                return (
                  <div key={a.key} className="flex items-center justify-between rounded-lg px-3 py-2"
                    style={{ background: `${a.color}08`, border: `1px solid ${a.color}20` }}>
                    <div className="flex items-center gap-2">
                      <Icon size={12} style={{ color: a.color }} />
                      <span className="text-[11px] text-white/60">{a.label}</span>
                    </div>
          ...
          <div>
            <span className="text-[10px] font-semibold tracking-widest text-white/30 uppercase">Coverage</span>
            <div className="flex flex-col gap-3 mt-3">
              {COVERAGE.map(c => (
                <div key={c.region}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[11px] text-white/50">{c.region}</span>
                    <span className="text-[11px] font-semibold" style={{ color: c.color }}>{c.pct}%</span>
                  </div>
